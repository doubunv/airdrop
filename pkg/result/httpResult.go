...
func HttpResult(r *http.Request, w http.ResponseWriter, resp interface{}, err error) {
	...
	if err == nil {
		...
		case errs.SystemError:
			httpx.WriteJson(w, http.StatusOK, Error(uint32(err.(errs.SystemError).Code()), err.Error()))
			return
		case errs.UserError:
			httpx.WriteJson(w, http.StatusOK, Error(uint32(err.(errs.UserError).Code()), err.Error()))
			return
		}

		// 错误返回
		errcode := xerr.SERVER_COxyzON_ERROR
		errmsg := "error"
