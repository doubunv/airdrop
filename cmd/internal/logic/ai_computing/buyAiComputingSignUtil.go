...
const buyAiComputingProjectSign = `{
    "types":{
        "EIP712Domain":[
          	{
                "name":"name",
    ...
    "primaryType":"BuyAiComputingProjectSign",
    "message":{
		"_orderId":"%d",
 		"_spentAmount":"%d",
 		"_orderTime":"%d",
...
func BuildAiComputingSign(config config.Config, order *schema.AiComputingOrder) (string, string) {
	...
	data := fmt.Sprintf(buyAiComputingProjectSign, chainID, contractAddr, order.ID, usdtNum, order.CreatedAt, order.UAddress)
