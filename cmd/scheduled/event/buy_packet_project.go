...
func (l *EventLogic) BuyPacketProject(chainTx *model.ChainTx) (err error) {
	var eventData EvenPacketData
	err = json.Unmarshal([]byte(chainTx.Data), &eventData)
	if err != nil {
		return errors.New(fmt.Sprintf("data Unmarshal err %v", err))
	}

	order, _ := l.svcCtx.PackageOrderModel.FindById(eventData.OrderId)
	if order.ID == 0 {
	...
	order.PayStatus = 2
	err = l.svcCtx.PackageOrderModel.UpdateSchema(&order)
	if err != nil {
		return err
	}
